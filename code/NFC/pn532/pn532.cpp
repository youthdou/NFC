...
bool PN532::wake_up()
{
    ...
    if(pn532_response_status == PN532_RESPONSE_ERROR)
        return false;
    if(pn532_response_packet.data[0] != 0x15)
        return false;
    else
        return true;
    return true;
...
bool PN532::pn532_get_firmware_version(pn532_version_t &version)
{
    ...
    if(pn532_response_packet.len != 6)
        return false;
    if(pn532_response_packet.data[0] != 0x03)
        return false;
...
{
    quint16 len;
    len = generate_cmd_frame((quint8 *)list_passive_target_cmd, sizeof(list_passive_target_cmd));
#if DEBUG
    show_cmd_packet(len);
#endif
    ...
    if(pn532_response_packet.data[0] != 0x4B)   //cmd + 1
        return false;

    if(pn532_response_packet.data[1] == 0)      //NbTg
        return false;

    if(pn532_response_packet.data[5] >= 20)     //NFCIDLength
        return false;

    QByteArray tmp((const char *)&pn532_response_packet.data[6],
            pn532_response_packet.data[5]);
    uid = tmp;
